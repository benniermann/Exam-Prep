...
\subsection{slist\_qsort}
\begin{lstlisting}
#include <stdlib.h>

int compare_slist(const void *a, const void *b)
{
    const SList *listA = (const SList *)a;
    const SList *listB = (const SList *)b;
...
\end{lstlisting}
\begin{lstlisting}
qsort(dein_array, anzahl_elemente, sizeof(SList), compare_slist);
\end{lstlisting}
...
\subsection{array\_to\_slist}
\begin{lstlisting}
SList *array_to_slist(const int *array, int size)
{
    if (array == NULL || size <= 0) {
        SList *empty_list = malloc(sizeof(SList));
        if (empty_list != NULL) {
