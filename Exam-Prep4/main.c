...
#define TEST_TASK_2 1   /* FIFO-Queue                      */
...
// ----------------------------------------------------------------------------
    ...
    remove("test_lines.txt"); remove("test_empty.txt"); remove("test_nonewline.txt");
    remove("test_ints.txt");  remove("test_max.txt");   remove("test_one.txt");
    remove("test_squares.txt"); remove("test_src.txt"); remove("test_dst.txt");
    remove("out.txt");
    printf("\n");
#endif
...
// ----------------------------------------------------------------------------
    ...
    {
        Queue *q = queue_create();
        for (int i = 0; i < 20; i++) queue_enqueue(q, i);
...
// ----------------------------------------------------------------------------
    ...
    {
        char *t;
        t = str_trim("  hallo  "); print_result("trim: '  hallo  ' -> 'hallo'", t && strcmp(t, "hallo") == 0); free(t);
        t = str_trim("\t test\n"); print_result("trim: '\\t test\\n' -> 'test'",  t && strcmp(t, "test")  == 0); free(t);
        t = str_trim("ok");        print_result("trim: 'ok' -> 'ok'",             t && strcmp(t, "ok")    == 0); free(t);
        t = str_trim("   ");       print_result("trim: nur Leerzeichen -> ''",    t && strcmp(t, "")      == 0); free(t);
        t = str_trim("");          print_result("trim: leerer String -> ''",       t && strcmp(t, "")      == 0); free(t);
    ...
    /* str_count_vowels */
    {
        ...
        print_result("count_vowels: 'aAbBiIoOuU' -> 10", str_count_vowels("aAbBiIoOuU") == 10);
    }

    /* str_to_upper */
    {
        char *u;
        u = str_to_upper("hallo Welt!"); print_result("to_upper: 'hallo Welt!' -> 'HALLO WELT!'", u && strcmp(u, "HALLO WELT!") == 0); free(u);
        u = str_to_upper("123abc");      print_result("to_upper: '123abc' -> '123ABC'",            u && strcmp(u, "123ABC")      == 0); free(u);
        u = str_to_upper("");            print_result("to_upper: '' -> ''",                         u && strcmp(u, "")            == 0); free(u);
    ...
    {
        char *r;
        r = str_repeat("ab", 3); print_result("repeat: 'ab' x3 -> 'ababab'", r && strcmp(r, "ababab") == 0); free(r);
        r = str_repeat("x",  1); print_result("repeat: 'x' x1 -> 'x'",       r && strcmp(r, "x")      == 0); free(r);
        r = str_repeat("hi", 0); print_result("repeat: 'hi' x0 -> ''",        r && strcmp(r, "")       == 0); free(r);
...
// ----------------------------------------------------------------------------
    ...
    {
        DynArray *a = da_create();
        da_push(a, 1.0); da_push(a, 2.0); da_push(a, 3.0); da_push(a, 4.0);
    ...
    {
        DynArray *a = da_create();
        da_push(a, 5.0); da_push(a, -2.0); da_push(a, 8.5); da_push(a, 1.0);
        ...
        da_min(c, &err);  print_result("da_min leer: err == -1", err == -1);
        da_max(c, &err);  print_result("da_max leer: err == -1", err == -1);
        da_mean(c, &err); print_result("da_mean leer: err == -1", err == -1);
        da_destroy(c);
    ...
    {
        DynArray *a = da_create();
        da_push(a, 3.0); da_push(a, 1.5); da_push(a, 4.0); da_push(a, 1.0); da_push(a, 5.0);
        ...
        DynArray *b = da_create();
        for (int i = 19; i >= 0; i--) da_push(b, (double)i);
        da_sort(b);
        int sorted = 1;
        for (int i = 0; i < 20; i++)
            if (fabs(da_get(b, i, &err) - (double)i) > 1e-9) sorted = 0;
